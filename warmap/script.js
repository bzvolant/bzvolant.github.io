// Initialize the map with a default view and disable attribution control
const map = L.map('map', { attributionControl: false }).setView([32.4279, 53.6880], 6);

// Add a tile layer to the map using MapTiler
...
fetch('locations.json')
    ...
    .then(data => {
        // Process the location data to ensure valid site types and parse coordinates
        const locations = data.map(location => ({
            ...location,
            siteType: ['oil', 'military', 'civilian', 'nuclear', 'utility', 'personofinterest'].includes(location.siteType) ? location.siteType : 'other',
            Coordinates: location.Coordinates ? location.Coordinates.split(',').map(c => parseFloat(c.trim())) : null
        }));

        // Define unique site types for filtering
        const uniqueTypes = ['oil', 'military', 'civilian', 'nuclear', 'utility', 'personofinterest', 'other'];
        const typeFilterContainer = document.getElementById('siteTypeFilterContainer');
        const timeFilterDropdown = document.getElementById('timeFilter');

        // Create checkboxes for each site type
        ...
        const updateMap = (filteredLocations) => {
            ...
            const showCasualtiesLabels = casualtiesCheckbox && casualtiesCheckbox.checked;
                ...
                fetch(svgPath, { method: 'HEAD' })
                    .then(response => {
                        ...
                        if (location.Coordinates && location.Coordinates.length === 2) {
                            ...
                            const totalInjured = (location.militaryCasualties ? parseInt(location.militaryCasualties.split(',')[0]) : 0) +
                                                (location.civilianCasualties ? parseInt(location.civilianCasualties.split(',')[0]) : 0) +
                                                (location.uncategorisedCasualties ? parseInt(location.uncategorisedCasualties.split(',')[0]) : 0);

                            const totalKilled = (location.militaryCasualties ? parseInt(location.militaryCasualties.split(',')[1]) : 0) +
                                               (location.civilianCasualties ? parseInt(location.civilianCasualties.split(',')[1]) : 0) +
                                               (location.uncategorisedCasualties ? parseInt(location.uncategorisedCasualties.split(',')[1]) : 0);

                            // Calculate total casualties for the location
                            const totalCasualties = totalInjured + totalKilled;

                            // Add total casualties to the pop-up content
                            const popupContent = `
                                <strong>${location.displayName}</strong><br>
                                ${location.Details ? `<em>${location.Details}</em><br>` : ''}
                                ${location.Province ? `<strong>Province:</strong> ${location.Province}<br>` : ''}
                                ${location.City ? `<strong>City:</strong> ${location.City}<br>` : ''}
                                ${location.Address ? `<strong>Address:</strong> ${location.Address}<br>` : ''}
                                ${location.date ? `<strong>Date:</strong> ${location.date}<br>` : ''}
                                ${location.time ? `<strong>Time:</strong> ${location.time}<br>` : ''}
                                ${location.militaryCasualties ? `<strong>Military Casualties:</strong> ${location.militaryCasualties.split(',')[0]} Injured, ${location.militaryCasualties.split(',')[1]} Killed<br>` : ''}
                                ${location.civilianCasualties ? `<strong>Civilian Casualties:</strong> ${location.civilianCasualties.split(',')[0]} Injured, ${location.civilianCasualties.split(',')[1]} Killed<br>` : ''}
                                ${location.uncategorisedCasualties ? `<strong>Uncategorised Casualties:</strong> ${location.uncategorisedCasualties.split(',')[0]} Injured, ${location.uncategorisedCasualties.split(',')[1]} Killed<br>` : ''}
                                ...
                                ${location.individuals && location.individuals.length > 0 ? `<strong>Individuals:</strong><br>${location.individuals.map(ind => `- ${ind.name} (${ind.role})`).join('<br>')}` : ''}
                            `;
                            ...
                            if (showCasualtiesLabels) {
                               ...
                               const labelMarker = L.marker([lat, lng], { icon: label }).addTo(map);
                            }
                        }
                    })
            });
        ...
        const filterLocations = () => {
            ...
            if (selectedTypes.length > 0) {
                filteredLocations = filteredLocations.filter(location => selectedTypes.includes(location.siteType));
            }
            if (selectedTime) {
        ...
        const calculateCasualtiesSum = (locations) => {
            ...
            locations.forEach(location => {
                if (location.militaryCasualties) {
                    const [injured, killed] = location.militaryCasualties.split(',').map(Number);
                    totalInjured += injured;
                    totalKilled += killed;
                }
                if (location.civilianCasualties) {
                    const [injured, killed] = location.civilianCasualties.split(',').map(Number);
                    totalInjured += injured;
                    totalKilled += killed;
                }
                if (location.uncategorisedCasualties) {
                    const [injured, killed] = location.uncategorisedCasualties.split(',').map(Number);
                    totalInjured += injured;
                    totalKilled += killed;
                }
            });

            console.log(`Total Injured: ${totalInjured}, Total Killed: ${totalKilled}`);
        };
        ...
        function setUIFromURL() {
            ...
            // Update checkboxes
            const typeCheckboxes = document.querySelectorAll('#siteTypeFilterContainer input[type=checkbox]');
        ...
        function setURLFromUI() {
            const selectedTypes = Array.from(document.querySelectorAll('#siteTypeFilterContainer input[type=checkbox]:checked')).map(cb => cb.value);
        ...
        function getFilteredLocations() {
            ...
            if (selectedTypes.length > 0) {
                filtered = filtered.filter(location => selectedTypes.includes(location.siteType));
            }
            if (selectedTime) {
                const daysAgo = parseInt(selectedTime, 10);
                const cutoffDate = new Date();
